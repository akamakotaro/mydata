...
マッハーツールへ組み込むための **PHP + ブラウザ完結** 版を [`mach-tools/`](./mach-tools/) に用意しています。
機能は同じで、バックエンドを PHP プロキシ（`api/states.php`）に置き換え、UI をマッハーツール風
（ダーク/ライト切替・カテゴリ「地図・検索」）に調整したものです。組み込み手順は
[`mach-tools/README.md`](./mach-tools/README.md) を参照してください。

```bash
cd mach-tools && php -S 127.0.0.1:8080
